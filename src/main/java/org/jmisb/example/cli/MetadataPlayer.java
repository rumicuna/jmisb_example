...
public class MetadataPlayer implements IMetadataListener, IFileEventListener {
    ...
    private void setUpErrorHandlers() {
        InvalidDataHandler.getInstance().setInvalidFieldEncodingStrategy(new PrintOnInvalidDataStrategy());
        InvalidDataHandler.getInstance().setInvalidChecksumStrategy(new PrintOnInvalidDataStrategy());
    }

    public void play(String filename) throws IOException {
        // These options configure for metadata decode only
        VideoFileInputOptions videoFileInputOptions = new VideoFileInputOptions(false, true, false, false);
    ...
    private void outputTopLevelMessageHeader(IMisbMessage misbMessage) {
        ...
        if (displayHeader.equalsIgnoreCase("Unknown")) {
            System.out.println(displayHeader + " [" + ArrayUtils.toHexString(misbMessage.getUniversalLabel().getBytes()).trim() + "]");
